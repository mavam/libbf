...
/// Retrieves  the minimum count of an item. This quering algorithm is dubbed
/// *minimum selection* (MS) in the spectral Bloom filter paper.
/// @tparam Store The store type.
...
size_t minimum(const typename Store::pos_vector& pos, const Store& store)
{
  auto min = store.max();
  for (auto i : pos)
  {
...
/// Retrieves the positions of all minima.
///
/// @tparam Store The store type.
///
/// @param pos A vector of cell indices of a store.
...
/// @param store An reference to a store.
///
/// @return a tuple *(min, rec, pos)* where *min* is the value of the minimum,
/// *rec* a boolean flag indicating whether the minimum is recurring, and *pos*
/// the vector containing all the positions of it. 
///
/// @note The *rec* flag is necessary because simply inspecting the number of
/// minima does not suffice: in case all @f$k@f$ hash functions yield the same
/// store position, we necessarily end up with one minimum and define this to
/// be a recurring one.
template <typename Store>
...
//    pos_min.shrink_to_fit();
  ...
  return std::make_tuple(min, recurring, pos_min);
}

/// Increases the cell of the minimum item. If there is no unique minimum then
/// increase all cells with the minimum. This quering algorithm is dubbed
/// *minimum increase* (MI) in the spectral Bloom filter paper.
/// @tparam T The type of the item to query.
/// @tparam Core The core type.
...
/// Retrieves the count of an item to according to the *recurring minimum* (RM)
/// algorithm from the spectral Bloom filter.
/// @tparam T The type of the item to query.
/// @tparam Core1 The type of the first core.
/// @tparam Core2 The type of the second core.
/// @param x An instance of type T.
...
/// Adds an item to the multiset according to the *minimum increase* (MI)
/// algorithm from the spectral Bloom filter.
/// @tparam T The type of the item to query.
/// @tparam Core1 The type of the first core.
/// @tparam Core2 The type of the second core.
...
/// Removes an item from the multiset according to the *minimum increase* (MI)
/// algorithm from the spectral Bloom filter.
/// @tparam T The type of the item to query.
/// @tparam Core1 The type of the first core.
/// @tparam Core2 The type of the second core.
